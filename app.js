...
class E Seller StoreApp {
  ...
  renderProductGrid(containerId, productList) {
    ...
    if (!productList || productList.length === 0) {
      container.innerHTML = `<p style="color:var(--text-muted); padding:20px;">No products found in this section.</p>`;
      return;
    }
        ...
        <div class="product-card">
          ...
          <div class="product-img-box">
            ...
            <div class="product-hover-actions">
              <button class="act-btn" title="Add to Compare" onclick="app.addToCompare('${prod.id}')">⚖️</button>
              <button class="act-btn" title="Quick View" onclick="app.openQuickView('${prod.id}')">👁️</button>
              <button class="act-btn" title="Wishlist" onclick="app.toggleWishlist('${prod.id}')">${isWishlisted ? '❤️' : '🤍'}</button>
              <button class="act-btn" title="Add to Cart" onclick="app.addToCart('${prod.id}')">🛒</button>
            </div>
          </div>

          <div class="product-card-body">
            <h4 class="product-title" title="${prod.name}">${prod.name}</h4>
            <div style="font-size:12px; color:#f59e0b; margin-bottom:6px;">⭐ ${prod.rating} (${prod.reviewsCount})</div>
