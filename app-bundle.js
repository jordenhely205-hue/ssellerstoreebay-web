...
class DokanEngine {
  ...
  saveProducts(products) {
    ...
    try {
      if (typeof idbStorage !== 'undefined' && idbStorage.putBatch) {
        idbStorage.putBatch('products', products);
      }
    } catch (e) {}

    // Cloud Database Persistence & Shared API Dispatch
    this.syncProductsToCloudBackend(products);

    // Invalidate & revalidate storefront cache
  ...
  submitVendorApplication({
    ...
    this.logActivity('New ' + (cleanRole === 'vendor' ? 'Vendor' : 'Seller') + ' Registration', 'Store ' + newApp.name + ' submitted application [PENDING APPROVAL]', 'warning');
    return newApp;
  }
...
class ESellerStoreApp {
  ...
  init() {
    ...
    try { this.initHeroSlider(); } catch (e) { console.error('initHeroSlider error:', e); }
    try { this.renderAll(); } catch (e) { console.error('renderAll error:', e); }
    try { this.setPersona('customer', false); } catch (e) { console.error('setPersona error:', e); }
    try { this.renderHomepageSections(); } catch (e) { console.error('renderHomepageSections error:', e); }
    try { this.renderCatalog(); } catch (e) { console.error('renderCatalog error:', e); }

    // Global asynchronous storefront data fetching directly from Server API / products.json
    if (typeof engine !== 'undefined' && engine.fetchServerProducts) {
      engine.fetchServerProducts().then(() => {
        this.renderHomepageSections();
  ...
  handleAdminSaveProduct(event) {
    ...
    try {
      ...
      const updatedProducts = engine.getProducts();
      try {
        localStorage.setItem('esellerstore_master_catalog', JSON.stringify(updatedProducts));
        localStorage.setItem('esellerstore_products', JSON.stringify(updatedProducts));
      } catch (err) {}
  ...
  handleAdminLogin(event) {
    ...
    const isPasswordMatch = (password === 'Abbas@123' || password === adminAuth.password);
  ...
  renderVendorDashboard() {
    ...
    const vendorOrders = allOrders.filter(o => {
      if (o.vendorId && o.vendorId === vendor.id) return true;
      if (Array.isArray(o.items)) {
        return o.items.some(item => 
          item.vendorId === vendor.id || 
          vendorProductIds.has(item.id) || 
          (item.sku && vendorProductSkus.has(item.sku))
        );
      }
      return false;
    });

    const pendingCount = vendorOrders.filter(o => o.status === 'Pending' || o.status === 'Processing' || o.status === 'Awaiting Payment Proof').length;
    const processingCount = vendorOrders.filter(o => o.status === 'Processing').length;
    const completeCount = vendorOrders.filter(o => o.status === 'Completed').length;
    const cancelCount = vendorOrders.filter(o => o.status === 'Cancelled').length;
    ...
    const vendorProfit = parseFloat(vendor.profitEarned || 0);
    const vendorBalance = parseFloat(vendor.balance || 0);
    ...
    if (statBal) statBal.textContent = '$' + totalRevenue.toFixed(2);
    if (statProf) statProf.textContent = '$' + vendorProfit.toFixed(2);
    ...
    const breakdownCompEl = document.getElementById('vendorStatusCountCompleted');
    if (breakdownPendEl) breakdownPendEl.textContent = pendingCount;
    if (breakdownProcEl) breakdownProcEl.textContent = processingCount;
    if (breakdownCompEl) breakdownCompEl.textContent = completeCount;

    const chartContainer = document.getElementById('vendorSalesChartContainer');
    const chartSubtext = document.getElementById('vendorChartStoreSubtext');

    if (chartSubtext) {
      chartSubtext.textContent = 'Real-time sales performance metrics for ' + vendor.name;
    }

    if (chartContainer) {
      if (vendorOrders.length === 0) {
        chartContainer.innerHTML = `
          <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:32px 16px; background:#f8fafc; border:1px dashed #cbd5e1; border-radius:8px; text-align:center;">
            <span style="font-size:32px; margin-bottom:8px;">📊</span>
            <strong style="color:#334155; font-size:14px; margin-bottom:4px;">No sales data recorded yet</strong>
            <p style="color:#64748b; font-size:12px; margin:0; max-width:340px; line-height:1.4;">
              Sales and performance charts will appear here automatically as customer orders are placed for <strong>${vendor.name}</strong>.
            </p>
          </div>
        `;
      } else {
        chartContainer.innerHTML = `
          <div class="vendor-chart-container">
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:35%;"></div><span class="vendor-chart-label">Feb</span></div>
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:55%;"></div><span class="vendor-chart-label">Mar</span></div>
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:45%;"></div><span class="vendor-chart-label">Apr</span></div>
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:70%;"></div><span class="vendor-chart-label">May</span></div>
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:85%;"></div><span class="vendor-chart-label">Jun</span></div>
            <div class="vendor-chart-bar-wrapper"><div class="vendor-chart-bar" style="height:100%; background:var(--nav-red);"></div><span class="vendor-chart-label">Jul (Current)</span></div>
          </div>
        `;
      }
    }
    ...
    const pagedOrders = vendorOrders.slice((currentOrderPage - 1) * orderPageSize, currentOrderPage * orderPageSize);

    const ordersBody = document.getElementById('vendorOrdersTableBody');
    if (ordersBody) {
      if (pagedOrders.length === 0) {
        ordersBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:24px; color:#64748b;">No customer orders placed yet for ' + vendor.name + '. Orders will appear here in real time.</td></tr>';
      } else {
    ...
    if (walletBalEl) walletBalEl.textContent = '$' + vendorBalance.toFixed(2);
    if (walletProfitEl) walletProfitEl.textContent = '$' + vendorProfit.toFixed(2);
    if (walletMarginEl) walletMarginEl.textContent = (vendor.profitMarginPercent || 25) + '% Net Margin';

    const walletLogsBody = document.getElementById('vendorWalletLogsTableBody');
    if (walletLogsBody) {
      const allWalletLogs = JSON.parse(localStorage.getItem('esellerstore_wallet_logs')) || [];
      const vendorLogs = allWalletLogs.filter(l => l.vendorId === vendor.id);
      if (vendorLogs.length === 0) {
        walletLogsBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:#64748b;">No wallet transactions recorded yet for ' + vendor.name + '.</td></tr>';
      } else {
        walletLogsBody.innerHTML = vendorLogs.map(l => `
          <tr>
            <td><strong>#${l.id || l.txId}</strong></td>
            <td>${l.date || 'Today'}</td>
            <td>${l.type ? l.type.toUpperCase() : 'TRANSACTION'} - ${l.note || 'Platform Adjustment'}</td>
  ...
  bindEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (personaBtn) {
        const persona = (personaBtn.dataset ? personaBtn.dataset.persona : personaBtn.getAttribute('data-persona')) || (personaBtn.textContent.includes('Seller') ? 'vendor' : (personaBtn.textContent.includes('Admin') ? 'admin' : 'customer'));
  ...
  // toggleMobileDrawer delegated to primary implementation
  ...
  // closeMobileDrawer delegated to primary implementation
